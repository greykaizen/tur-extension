...
use windows::Win32::Graphics::Gdi::{
    BeginPaint, ClientToScreen, CombineRgn, CreateRectRgn, DeleteObject, EndPaint, SetWindowRgn,
    PAINTSTRUCT, RGN_OR,
};
...
struct GpuState {
    ...
    text_layout: IDWriteTextLayout, // Pre-created layout for "Download with tur"
    x_layout: IDWriteTextLayout,    // Pre-created layout for "x"
...
unsafe fn do_update(u: CanvasUpdate) {
    ...
    let new_hwnd = if let Some(hwnd) = existing_hwnd {
        ...
        let ex_style = WINDOW_EX_STYLE(
            WS_EX_TOPMOST.0 | WS_EX_TOOLWINDOW.0 | WS_EX_NOREDIRECTIONBITMAP | WS_EX_NOACTIVATE.0,
        );
        CreateWindowExW(
            ex_style,
            w!("TurOverlayCanvas"),
...
unsafe fn init_gpu(hwnd: HWND, w: u32, h: u32, dpi: f32) -> windows::core::Result<GpuState> {
    // ── D3D11 device ─────────────────────────────────────────────────────────
    ...
    let text_fmt: IDWriteTextFormat = dwrite.CreateTextFormat(
        w!("Instrument Sans"), // Render in Le Murmure custom font
        font_collection_ref,
